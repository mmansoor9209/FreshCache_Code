...
SCHEDULE, SEED, N_SAMPLE = "zipf_uniform", 42, 300
CLASSES = ["TIMELESS", "SLOW", "MEDIUM", "FAST", "REAL_TIME"]
NOL1, STTL = "FreshCache_NoL1", "SemanticTTL_k1_16"
L1ONLY, EXACT = "FreshCache_L1Only", "ExactTTL"
POLICIES = ["NoCache", EXACT, STTL, L1ONLY, NOL1, "FreshCache"]
...
def main():
    ...
    print(f"  sampling frame restricted to {len(TEST_C):,} HELD-OUT TEST "
    ...
    for pos in range(half, len(stream)):
        ...
        if not all(u["url_hash"] in url_of for u in r["urls"]):
            continue
        if (r.get("cluster_id") or r["query_id"]) not in TEST_C:
            continue
        seen.add(r["query_id"])
    ...
    if len(want) < N_SAMPLE:                # top up across classes if a class is short
        pool = sorted({q for c in CLASSES for q in by[c]} - set(want))
        rng.shuffle(pool)
        want.extend(pool[:N_SAMPLE - len(want)])
    ...
    cfgs = {
        ...
        L1ONLY: dict(use_l1=True, use_l2=False, use_l3=False, temporal="risk"),
    }
    ea.CONFIG[L1ONLY] = {**base, "l2": "none", "l3": "none",
                         "l2_reg_on_hit": False}
    me.VARIANTS[L1ONLY] = (L1ONLY, "risk")
    ...
    ecap = _copy.deepcopy(cap)
    for v in ecap.values():
        v["path"] = "exact_miss"
    specs[EXACT] = ecap
    aggr[EXACT] = {"note": "ExactTTL realises 2 L1 hits in 31,201 requests "
                           "(see 03_exactttl/); every sampled held-out request "
                           "is an exact-match MISS, so its measured path is a "
                           "normalised-string lookup followed by the full "
                           "search+fetch+generate path"}
